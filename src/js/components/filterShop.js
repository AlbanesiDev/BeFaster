function filterShop(products) {
    ...
    // searchBar.onchange = performSearch;
    ...
    function applyFilters() {
        const filteredProducts = products.filter(product => {
            const categoryMatch = selectedCategories.size === 0 || selectedCategories.has(product.category.toLowerCase());
            const genderMatch = selectedGenders.size === 0 || selectedGenders.has(product.gender.toLowerCase());
            const sizesMatch = selectedSizes.size === 0 || product.size.some(size => selectedSizes.has(size));
            const colorsMatch = selectedColors.size === 0 || product.color.some(color => selectedColors.has(color.toLowerCase()));
